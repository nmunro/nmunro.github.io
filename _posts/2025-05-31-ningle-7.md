...
- [Part 15 (Pagination, Part 2)](/2026/02/28/ningle-15.html)
...
Note: My package (envy-ningle) is NOT in [quicklisp](https://www.quicklisp.org/beta/), so you will need to clone it using `git` into your `local-packages` directory. Please ensure that you use the tag [v0.0.1](https://github.com/nmunro/envy-ningle/releases/tag/v0.0.1) for this tutorial, it is a package I work on and use in my own projects, so the latest may not necessarily work for any given chapter. I will try to ensure the correct versions are specified, but if you find a mistake [let me know](mailto:nmunro@duck.com).
...
Note: Please _do_ make sure to use the `(or ...)` form, this will ensure that the file can compile correctly.
